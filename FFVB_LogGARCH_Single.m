function results = FFVB_LogGARCH_Single(dataFile, seed)

fprintf('FFVB for Log-GARCH(1,1) - 2015–2025\n');

% default seed and optional input handling
...
% Model configuration
fprintf('2. Initializing model and variational parameters...\n');
...
post = perform_posterior_analysis(variational_opt, config.post_analysis);

% show results
display_optimized_results(post, variational_opt, elbo_history);

% Volatility forecasting
fprintf('5. Forecasting volatility...\n');
[vol1, vol2] = forecast_volatility_ffvb(post.stats, config.params);
fprintf('   1-step forecast: %.4f\n', vol1);
fprintf('   2-step forecast: %.4f\n', vol2);
fprintf('5. Forecasting volatility...\n');
...
% plots (skip if no valid ELBO history)
if ~isempty(elbo_history) && length(elbo_history) > 5 && ~any(isnan(elbo_history))
    create_optimized_plots(elbo_history, m_history, post);
else
    fprintf('   No valid optimization history, skipping plots\n');
    figure; plot(1:10, -1000*ones(1,10), 'LineWidth', 2);
    title('ELBO (no data)'); xlabel('Iteration'); ylabel('ELBO');
end

fprintf('\n=== FFVB Analysis Complete! ===\n');

% Pack outputs into a results struct (match shape of FFVB_LogGARCH_Single.m)
results = struct();
results.post = post;                      % posterior samples & stats
results.variational = variational_opt;    % optimized variational parameters
results.elbo_history = elbo_history;
results.m_history = m_history;
results.forecast.vol1 = vol1;
results.forecast.vol2 = vol2;
...
% Configuration functions for Log-GARCH

function config = get_model_config(T, y)
    % Model parameter configuration for Log-GARCH
    config.params.T = T;
    config.params.y = y;
...
function variational = initialize_variational_parameters(y)
    % Initialization for Log-GARCH variational parameters
...
% ELBO computation for Log-GARCH with numerical stability
...
while valid_count < L && attempt < max_attempts
    ...
    % Direct parameter usage for Log-GARCH (no tanh transformation)
    % Log-GARCH parameters don't need non-negativity constraints
...
% Joint probability computation for Log-GARCH model
% Log-GARCH models do not require non-negativity constraints on parameters
...
eps = params.epsilon;
    ...
    % Parameter validity check for Log-GARCH
...
if ~success_lik || ~isfinite(log_lik)
    ...
    % Prior for Log-GARCH - weakly informative priors
...
% Log-likelihood computation for Log-GARCH(1,1) model
...
eps = params.epsilon;
    ...
    % Log-GARCH model specification
    % Log-GARCH can handle negative parameters without non-negativity constraints
log_sigma = zeros(T, 1);
epsilon_t = zeros(T, 1);
...
for t = 2:T
    epsilon_prev = (y(t-1) - mu) / (exp(log_sigma(t-1)/2) + eps);
...
for t = 1:T
    ...
    % Analytical gradients for Log-GARCH
grad = zeros(4, 1);
if nargout > 1
...
% Weakly informative priors for Log-GARCH parameters
...
% Variational distribution density with stability checks
...
% Posterior analysis for Log-GARCH
...
% Sampling from variational posterior
...
% Reconstruct log(sigma_t^2) series
log_sigma2 = zeros(T, 1);
epsilon = zeros(T, 1);

% Stable initialization
initial_variance = mean((y - mu).^2);
log_sigma2(1) = log(initial_variance + eps);

for t = 2:T
    sigma_prev = exp(log_sigma2(t-1)/2);
    epsilon(t-1) = (y(t-1) - mu) / (sigma_prev + eps);
    u_prev = abs(epsilon(t-1)) - sqrt_2_pi;
    log_sigma2(t) = omega + alpha * u_prev + beta * log_sigma2(t-1);
...
sigma_T = exp(log_sigma2(T)/2);
epsilon_T = (y(T) - mu) / (sigma_T + eps);
u_T = abs(epsilon_T) - sqrt_2_pi;
log_sigma2_1step = omega + alpha * u_T + beta * log_sigma2(T);
vol1 = exp(log_sigma2_1step/2);

% 2-step ahead forecast
% For 2-step, we use E[|epsilon|] = sqrt(2/pi), so u = E[|epsilon|] - sqrt(2/pi) = 0
log_sigma2_2step = omega + alpha * 0 + beta * log_sigma2_1step;
vol2 = exp(log_sigma2_2step/2);

end

function create_optimized_plots(elbo_history, m_history, results)
% Plotting with robust handling

figure('Position', [100, 100, 1400, 900], 'Color', 'white');

% ELBO convergence - handle negative values properly
subplot(2, 3, 1);
...
sgtitle('FFVB for Log-GARCH(1,1) - Results', 'FontSize', 14, 'FontWeight', 'bold');
set(gcf, 'Color', 'white');
